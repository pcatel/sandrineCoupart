        ...
        <div class="container-xxl py-5"> 
            ...
            <div class="row g-5">
                ...
                <div class="col-lg-7 wow fadeIn" data-wow-delay="0.5s">
                    ...
                    $query = "SELECT * FROM regimes WHERE id = :id";
                    $stmt = $conn->prepare($query);
                    $stmt->bindParam(':id', $idRegime, PDO::PARAM_INT);
                    $stmt->execute();
                    $regime = $stmt->fetch(PDO::FETCH_ASSOC);

                    // Vérification si la allergie existe
                    if (!$regime) {
                        echo "Ce régime n'existe pas.";
                        exit;
                    }

                    // Affichage du formulaire de modification de la regimes
                    echo '<form action="modifier_regime.php" method="POST">';
                    echo '<div class="table-responsive">';
                    echo '<table  class="table table-hover">';

                    echo '<input type="hidden" name="idRegime" value="' . $regime['Id'] . '">';

                    echo '<tr><td><div class="form-outline"><label for="nom">Nom :</label></td>';
                    echo '<td><input type="text" name="nom" value="' . $regime['nom'] . '" class="formField"></td></tr></div>';


                    echo '<tr><td><div class="form-outline "><label for="description">Description :</label></td>';
                    echo '<td><textarea name="description" class="formField">' . $regime['description'] . '</textarea></td></tr></div>';
